...
export default function MyOrdersScreen({ navigation }: any) {
  ...
  // ── Refresh list every time the tab comes into focus ──────
  useFocusEffect(
    useCallback(() => {
      loadOrders();
    }, [loadOrders])
        ...
        (payload) => {
          if (payload.eventType === 'UPDATE') {
            ...
            if (updated.status === 'ready') {
              Alert.alert(
                '🍽️ Order Ready!',
                `Order #${updated.id.slice(-6).toUpperCase()} is ready to be served!`
              );
            }
            setOrders((prev) =>
              ...
              <>
                ...
                {activeOrders.map((order, index) => (
                  <ActiveOrderCard
                    ...
                    onCancel={
                      order.status === 'pending' ? () => handleCancelOrder(order) : undefined
                    }
                  />
                ))}
              </>
          ...
          <View style={styles.modal}>
            {detailOrder &&
              (() => {
                ...
                const canRate =
                  detailOrder.status === 'completed' && !ratedOrders.has(detailOrder.id);
                return (
                  <>
                    ...
                    <View style={styles.modalBtnCol}>
                      {canCancel && (
                        <Button
                          title="Cancel Order"
                          onPress={() => handleCancelOrder(detailOrder)}
                          variant="danger"
                        />
                      )}
                      {canRate && (
        ...
        <View style={styles.historyInfo}>
          ...
          {isCompleted && (
            <Text
              style={[
                styles.historyRated,
                { color: isRated ? theme.colors.success : theme.colors.textMuted },
              ]}
            >
