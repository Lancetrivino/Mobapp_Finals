...
export default function OrderManagementScreen() {
  ...
  const advanceStatus = async (order: Order) => {
    ...
    if (success) {
      // Optimistically update detail modal if open
      if (detailOrder?.id === order.id) {
        setDetailOrder({ ...order, status: nextStatus });
      }
      loadOrders();
    }
  };
  ...
  const cancelOrder = async (order: Order) => {
    Alert.alert('Cancel Order', `Cancel Order #${order.id.slice(-6).toUpperCase()}?`, [
  ...
  const deleteOrder = async (order: Order) => {
    Alert.alert('Delete Order', `Permanently delete #${order.id.slice(-6).toUpperCase()}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
        onPress: async () => {
          const success = await storage.deleteOrder(order.id);
          if (success) {
            setDetailOrder(null);
            loadOrders();
          } else {
                ...
                <>
                  ...
                  {detailOrder.table_number != null && (
                    <Text style={styles.modalMeta}>
                      🪑 Table {detailOrder.table_number}
                    </Text>
                  )}
                  {detailOrder.notes ? (
                  ...
                  {/* Items — with real names from the join */}
                  ...
                  <View style={styles.modalActions}>
                    {canAdvance && (
                      <Button
                        ...
                        onPress={() => advanceStatus(detailOrder)}
                        variant="success"
                      />
                    )}
                    {canAdvance && (
                      <Button
                        title="Cancel Order"
                        onPress={() => cancelOrder(detailOrder)}
                        variant="danger"
                      />
                    )}
                    <Button
                      title="Delete Order"
                      onPress={() => deleteOrder(detailOrder)}
                      variant="outline"
                    />
                    <Button
                      title="Close"
...
const KanbanCard: React.FC<{
  ...
  return (
    <Animated.View
      style={[
        styles.kanbanCard,
        { opacity: opacityAnim, transform: [{ translateY: entryY }, { translateX: slideX }], borderLeftColor: colColor },
      ]}
    >
      <TouchableOpacity onPress={onTap} activeOpacity={0.85}>
        <View style={styles.kanbanCardHeader}>
          {order.table_number != null && (
            <View style={[styles.tableBadge, { backgroundColor: colColor + '25' }]}>
              <Text style={[styles.tableBadgeText, { color: colColor }]}>
        ...
        {/* Show actual item names — no more "Item #1" */}
        <View style={styles.kanbanItems}>
          {order.items.map((item, idx) => (
            <View key={idx} style={styles.kanbanItemRow}>
              <Text style={styles.kanbanItemName} numberOfLines={1}>
                {item.quantity}× {item.name ?? `Item ${idx + 1}`}
              </Text>
              <Text style={styles.kanbanItemPrice}>
      ...
      {canAdvance && (
        <TouchableOpacity
          style={[styles.advanceBtn, { backgroundColor: colColor }]}
...
const styles = StyleSheet.create({
  ...
  kanbanScroll: { paddingHorizontal: theme.spacing.lg, paddingBottom: 24, gap: theme.spacing.md },
  column: {
    width: COLUMN_WIDTH, backgroundColor: theme.colors.surface, borderRadius: theme.borderRadius.large,
    borderWidth: 1, borderColor: theme.colors.border, padding: theme.spacing.sm,
    maxHeight: '88%', ...theme.shadows.small,
  },
  columnHeader: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingBottom: theme.spacing.sm, marginBottom: theme.spacing.sm, borderBottomWidth: 2,
  },
  columnTitle: { fontSize: 14, fontWeight: '800', letterSpacing: 0.3 },
  columnBadge: { width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  columnBadgeText: { fontSize: 12, fontWeight: '800' },
  columnEmpty: { alignItems: 'center', paddingVertical: 32, gap: 8 },
  columnEmptyText: { fontSize: 12, color: theme.colors.textMuted },
  kanbanCard: {
    backgroundColor: theme.colors.surfaceHigh, borderRadius: theme.borderRadius.medium,
    padding: theme.spacing.sm, marginBottom: theme.spacing.sm, borderLeftWidth: 3, ...theme.shadows.small,
  },
  kanbanCardHeader: {
    flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8, flexWrap: 'wrap',
  },
  tableBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  ...
  kanbanItemName: { fontSize: 12, color: theme.colors.text, fontWeight: '600', flex: 1, marginRight: 4 },
  ...
  advanceBtn: {
    ...
    gap: 6, paddingVertical: 8, borderRadius: theme.borderRadius.small, marginTop: 8,
  },
  ...
  statusBadge: {
    ...
    paddingHorizontal: 12, paddingVertical: 6, borderRadius: theme.borderRadius.full, marginBottom: theme.spacing.md,
  },
  statusBadgeText: { fontSize: 13, fontWeight: '700' },
  modalMeta: { fontSize: 13, color: theme.colors.textSecondary, marginBottom: 6 },
  modalSection: {
    fontSize: 13, color: theme.colors.textSecondary, fontWeight: '700',
    marginBottom: theme.spacing.sm, marginTop: theme.spacing.xs, textTransform: 'uppercase', letterSpacing: 0.6,
  },
  detailItemRow: {
    flexDirection: 'row', justifyContent: 'space-between',
    paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: theme.colors.border,
  },
  detailItemText: { fontSize: 14, color: theme.colors.textSecondary, flex: 1, marginRight: 8 },
